...
// Adapted from ec-key 0.0.4 by Pier Fumagalli (https://github.com/usrz/ec-key)
// Add SM2 curve so that SM2 key can be recognized.
// Peter Liu
// June 15, 2020
 ...
 * | sm2       |            | NIST SM2   | sm2        | 1 2 156 10197 1 301 | *
 * +-----------+------------+------------+------------+---------------------+ *
 * ========================================================================== */
...
function parsePkcs8(buffer) {
    let key = ASN1ECPkcs8Key.decode(buffer, 'der');
    let privateKeyWrapper = ASN1ECRfc5915Key.decode(key.privateKey, 'der');
    let curve = key.algorithmIdentifier.parameters;
    let bytes = lengths[curve];

    let privateKey = privateKeyWrapper.privateKey;
    if (privateKey.length > bytes) {
        throw new TypeError('Invalid private key size: expected ' + bytes + ' gotten ' + privateKey.length);
    }

    let components = parsePublicKeyBuffer(curve, privateKeyWrapper.publicKey.data);
...
function parseRfc5915(buffer) {
    let key = ASN1ECRfc5915Key.decode(buffer, 'der');
    let bytes = lengths[key.parameters];
    ...
    let components = parsePublicKeyBuffer(key.parameters, key.publicKey.data);
    components.d = privateKey;
    return components;
}
...
function parseSpki(buffer) {
    let key = ASN1ECSpkiKey.decode(buffer, 'der');
...
function parsePem(pem) {
    ...
    if (match = pem.match(pemRfc5915RE)) {
        const buffer = new Buffer(match[1].replace(/[\s-]/mg, ''), 'base64');
...
function ECKey(key, format) {
    ...
    if (util.isBuffer(key)) {
        ...
        if (format === 'pem') {
            const k = parsePem(key.toString('ascii'));
            curve = k.c;
            x = k.x;
            y = k.y;
            d = k.d;
    ...
    // STRING KEYS: base64 all the time, but also allowed in PEM
        ...
        if (format === 'pem') {
            ...
            const k = parsePkcs8(new Buffer(key, 'base64'));
            curve = k.c;
            x = k.x;
            y = k.y;
            d = k.d;
            ...
            const k = parseSpki(new Buffer(key, 'base64'));
            curve = k.c;
            x = k.x;
            y = k.y;
            d = k.d;
    ...
    // private key and "(publicKey|x,y)" always required (both for priv and pub)
        ...
        if (util.isBuffer(key.publicKey)) {
            const k = parsePublicKeyBuffer(curve, key.publicKey);
            x = k.x;
            y = k.y;

        } else if (util.isString(key.publicKey)) {
            const k = parsePublicKeyBuffer(curve, new Buffer(key.publicKey, 'base64'));
            x = k.x;
            y = k.y;
...
ECKey.prototype.createECDH = function createECDH() {
    if (this.isPrivateECKey) {
        let ecdh = crypto.createECDH(this.curve);
        ecdh.setPublicKey(this.publicCodePoint);
        ecdh.setPrivateKey(this.d);

        let ecdhComputeSecret = ecdh.computeSecret;
...
ECKey.prototype.createSign = function createSign(hash) {
    ...
    let sign = crypto.createSign('RSA-' + hash); // RSA works with EC keys, too
    let signFunction = sign.sign;
    let self = this;
...
ECKey.prototype.createVerify = function createVerify(hash) {
    let verify = crypto.createVerify('RSA-' + hash); // RSA works with EC keys, too
    let verifyFunction = verify.verify;
    let key = this.isPrivateECKey ? this.asPublicECKey() : this;
...
ECKey.prototype.toJSON = function toJSON() {
    ...
    let d = this.d;
    if (d) {
        let bytes = lengths[this.curve];
        if (d.length < bytes) {
            let remaining = bytes - d.length;
